...
class DatabaseMinifierAdv {
	 ...
	 * @param array $extraLimits
	 */
	public function __construct( array $connectionsConfigs, array $extraRelations = [], array $extraLimits = [], array $extraFilters = [] ) {
	 ...
	 * @param int $limit => set default 1000 record
	 * @param array $fields
	 *
	 * @return array
	 * @throws DatabaseMinifierException
	 */
	protected function prepareSql( $tableName, $criteria = [], $limit = 100, $fields = [] ) {
		$conditions = [];
		$params     = [];
	...
	public function copyRecordsByCriteria(
		...
		if ( $limit == 0 ) {
			$limit = $this->getLimitByTableName( $tableName );
		}

		// Get filter from config
		$filters = $this->getFiltersByTableName( $tableName );
		if ( ! empty( $filters ) ) {
	...
	public function getLimitByTableName( $tableName ) {
		$limit = 0;

		if ( array_key_exists( $tableName, $this->extraLimits ) ) {
			$limit = $this->extraLimits[ $tableName ];
		}

		return $limit;
	}

	public function getFiltersByTableName( $tableName ) {
		$filters = [];

		if ( array_key_exists( $tableName, $this->extraFilters ) ) {
			$filters = $this->extraFilters[ $tableName ];
		}
